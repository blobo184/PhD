...
run_mods <- function(sample, year, lambda) {
  ...
  if (year == 2012){
    ...
    current_mod <- "2012"
    historic_dat <- "anes_2008_tidy.RData"
    current_dat <- "anes_2012.RData"
  } else if (year == 2016){
    historic_mod <- "mod_2012.rds"
    current_mod <- "2016"
    historic_dat <- "anes_2012_tidy.RData"
  ...
  if(file.exists(paste0("../Results/Historic/", historic_mod))==FALSE){
    
    # Load data
    load(paste0("../Data/", historic_dat))
    ...
    # Save model
    saveRDS(mod, file = paste0("../Results/Historic/", historic_mod))
    rm(mod, dat)
  }
  ...
  if(file.exists(paste0("../Results/", sample, "mod_", current_mod, inf_mod))==FALSE){
    ...
    source("00_Utility.R")
    mod <- readRDS(paste0("../Results/Historic/", historic_mod))
